...
interface EnemyCardProps {
  ...
  onAddEnemy: (enemy: { name: string; maxHp: number }) => void;
}
    ...
    <>
      ...
      `}>
        ...
        <button
          ...
          className="mt-4 px-4 py-2 bg-amber-700 hover:bg-amber-600 text-white rounded"
        >
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
            <div className="bg-amber-800 p-6 rounded-lg">
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                  <label className="block text-white mb-2">Points de vie max</label>
                  <input
                    type="number"
                    value={newEnemy.maxHp}
                    onChange={(e) => setNewEnemy({...newEnemy, maxHp: parseInt(e.target.value)})}
                    className="w-full p-2 rounded"
                    required
                  />
                </div>
